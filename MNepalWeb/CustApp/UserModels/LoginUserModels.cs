...
namespace CustApp.UserModels
{
    public class LoginUserModels : IUserDbService
    {
                    ...
                    using (SqlCommand cmd = new SqlCommand("[s_MNCheckLinkBankAcc]", conn))
                    {
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            using (DataSet dataset = new DataSet())
                            {
                                ...
                                if (dataset.Tables.Count > 0)
                                {
                                    if (dataset.Tables.Contains("Table2"))
                                    {
        ...
        #region Get Bank Register Name
        public DataTable GetBankRegisterName(UserInfo objUserInfo)
        {
            DataTable dtableResult = null;
            SqlConnection conn = null;
            try
                    ...
                    using (SqlCommand cmd = new SqlCommand("[s_MNCheckLinkBankAcc]", conn))
                    {
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            using (DataSet dataset = new DataSet())
                            {
                                ...
                                if (dataset.Tables.Count > 0)
                                {
                                    if (dataset.Tables.Contains("Table3"))
                                    {
                                        dtableResult = dataset.Tables[3];
                                    }
                                }
                            }
                        }
