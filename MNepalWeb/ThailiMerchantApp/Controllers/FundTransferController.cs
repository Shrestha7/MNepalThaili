    ...
    public class FundTransferController : Controller
    {
            ...
            if (reqToken == "0")
            {
                ...
                if (_ft.sc == "00" || _ft.sc == "11")
                {
                    if (_ft.da == userName)
                    {
                        ...
                        return Json(new { responseCode = "", responseText = "Fund transfer is not allowed to same user", blockMessage = BlockMessage },
        ...
        public ActionResult GetCheckingUserName(string Username)
        {
            if ((Username != "") || (Username != null))
            {
                ...
                if (dtableMerchantMobileNo.Rows.Count != 0)
                {
                ...
                {
                    DataTable dtableCustMobileNo = RegisterUtils.GetCheckMobileNo(Username);
                    if (dtableCustMobileNo.Rows.Count == 0)
                    {
                        result = "Error";
                        return Json(result, JsonRequestBehavior.AllowGet);
                    }
