...
namespace ThailiMerchantApp.Controllers
{
    ...
    public class MerchantDashboardController : Controller
    {
...

        {
            ...
            string QRCodeImgPath = GenerateQRCode();
        ...
        //For QRCode
        private string GenerateQRCodePath(string qrcodeText, string userName)
        {
            string folderPath = "~/Images/";
            string imagePath = "~/Images/" + "QRCode_" + userName + ".jpg"; //QrCode.jpg
        ...
        public string GenerateQRCode()
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                try
                {
                    String EncKey = "EncryptthailiKey";
                    string QRCodeText = userName + ',' + userInfo.UserType + ',' + userInfo.MerchantCategory + ',' + userInfo.MerchantId + ',' + userInfo.MName;
                    QRCodeText = Encrypter.EncryptData(EncKey, QRCodeText);
                    string QRCodeImagePath = GenerateQRCodePath(QRCodeText, userName);
