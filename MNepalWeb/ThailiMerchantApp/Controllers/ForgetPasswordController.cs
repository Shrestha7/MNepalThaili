    ...
    public class ForgetPasswordController : Controller
    {
        ...
        public ActionResult GetCheckingUserName(string username)
          {
            ...
            if ((username != "") || (username != null))
            {
                ...
                string BlockMessage = LoginUtils.GetMessage("03");
                string AccountExistsMessage = LoginUtils.GetMessage("04");
                if (dtableMobileNo.Rows.Count == 1)
                {
                    if (dtableBlockUserWrongPwd != null && dtableBlockUserWrongPwd.Rows.Count > 0)
                    {
                        result = "Success";
                        // return Json(result, JsonRequestBehavior.AllowGet);
                        return Json(new { result = result, blockMessage = BlockMessage, accountExistsMessage = AccountExistsMessage }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Recover(FormCollection collection)
        {
            ...
            try
            {
                ...
                {
                    displayMessage = "VerificationCode doesnot not Match";
                    messageClass = CssSetting.FailedMessageClass;
                }
            }
            catch (Exception ex)
