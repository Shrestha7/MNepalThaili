...
namespace ThailiMNepalAgent.Controllers
{
    public class NepalWaterController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> NepalWaterPayment(NepalWater KP)
        {
            ...
            if (reqToken == "0")
            {
                ...
                Session["NWCounter"] = KP.NWCounter;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var content = new FormUrlEncodedContent(new[]{
                        ...
                        new KeyValuePair<string, string>("serviceCode", KP.NWCounter),
                        new KeyValuePair<string, string>("account", KP.CustomerID),

                        //new KeyValuePair<string, string>("serviceCode","2"),
                        //new KeyValuePair<string, string>("account","1234567"),
        ...
        public ActionResult Details()
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                NepalWater KPObj = new NepalWater();
                KPObj.NWCounter = S_NWCounter;
                ...
                if (dResponse != null && dResponse.Rows.Count > 0)
                {
                    ...
                    else
                    {
                      return RedirectToAction("Index");
                        ...
                        //double TotalAmountDue1 = Convert.ToDouble(regobj.totalAmount.ToString());
                        //TotalAmountDue1 = TotalAmountDue1 / 100;
                        ////end Converting paisa to rupee
                        ////splitting decimal values 
                        //int S_TotalAmountDue = Convert.ToInt32(TotalAmountDue1);
                        //String[] Str_TotalAmountDue = TotalAmountDue1.ToString().Split('.');
                        //if (Str_TotalAmountDue.Length == 2)
                        //{
                        //    S_TotalAmountDue = Convert.ToInt32(TotalAmountDue1.ToString().Split('.')[0]) + 1;//adding 1 to decimal value
                        //}
                        ////end
                        //ViewBag.S_TotalAmountDue = S_TotalAmountDue;
                        //ViewBag.TotalAmountDue = TotalAmountDue1;
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public async Task<ActionResult> NWExecutePayment(NepalWater KP)
        {
            ...
            if (reqToken == "0")
            {
                ...
                //DataTable dtableUserInfo = PaypointUtils.GetKPDetails(KPObj);
                DataSet DPaypointSet = PaypointUtils.GetNWDetails(KPObj);
                DataTable dResponse = DPaypointSet.Tables["dtResponse"];
                DataTable dNWPayment = DPaypointSet.Tables["dtNWPayment"];
                if (dResponse != null && dResponse.Rows.Count > 0)
        ...
        public string getrefStan(NepalWater KPObj)
        {
            string Query_refStan = "select refStan from MNPaypointResponse where account='" + KPObj.CustomerID + "' AND serviceCode='" + KPObj.NWCounter + "' AND ClientCode='" + KPObj.ClientCode + "' AND UserName='" + KPObj.UserName + "'  order by transactionDate";
            DataTable dt = new DataTable();
