    ...
    public class CustomerController : Controller
    {
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (btnApprove.ToUpper() == "REJECT")
                {
                    ...
                    if (ret == 100)
                    {
                        ...
                        displayMessage = "Customer " + userInfoApproval.FName + " has been Rejected. Please Check Wallet Customer Status and perform accordingly";
                        messageClass = CssSetting.SuccessMessageClass;
                        //SMS
                        SMSUtils SMS = new SMSUtils();
                ...
                if (btnApprove.ToUpper() == "APPROVE")
                {
                    ...
                    if (ret == 100)
                    {
                        ...
                        displayMessage = "Customer Information for  user " + userInfoApproval.FName + " has successfully been approved.";
        ...
        public ActionResult ViewSelfRegDetail(string id)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (dtableSrInfo != null && dtableSrInfo.Rows.Count > 0)
                {
                    ...
                    srInfo.Name = dtableSrInfo.Rows[0]["Name"].ToString();
        ...
        public ActionResult CustomerDetails(string SearchCol, string txtMobileNo)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                {
                    DataTable dtableCustomerStatusByMobileNo = CustomerUtils.GetApprovedCustKYC(userInfo.ContactNumber1);
        ...
        public ActionResult CustomerModify(string id)
        {
            ...
            if (this.TempData["Customertmodify_messsage"] != null)
            {
                this.ViewData["Customertmodify_messsage"] = this.TempData["Customertmodify_messsage"];
                this.ViewData["message_class"] = this.TempData["message_class"];
            }
            ...
            if (TempData["userType"] != null)
            {
                ...
                DataTable dtableSrInfo = DSet.Tables["dtUserInfo"];
                ...
                if (dtableSrInfo != null && dtableSrInfo.Rows.Count > 0)
                {
                    ...
                    srInfo.PProvince = getProvinceName(dtableSrInfo.Rows[0]["PProvince"].ToString());
                    srInfo.PDistrict = getDistrictName(dtableSrInfo.Rows[0]["PDistrict"].ToString());
                    ...
                    srInfo.CProvince = getProvinceName(dtableSrInfo.Rows[0]["CProvince"].ToString());
                    srInfo.CDistrict = getDistrictName(dtableSrInfo.Rows[0]["CDistrict"].ToString());
        ...
        [HttpPost]
        public ActionResult CustomersModify(UserInfo model, UserInfo info, string btnApprove, FormCollection collection, string Remarks)
        {
            string Id = "";
            ...
            try
            {
                ...
                if (TempData["userType"] != null)
                {
                    ...
                    UserInfo userKYCApproval = new UserInfo();

                    if (btnApprove.ToUpper() == "RESET KYC")
                    {
                        userKYCApproval.ClientCode = info.ClientCode;
                        userKYCApproval.UserName = info.UserName;
                        userKYCApproval.Name = info.Name;
                        userKYCApproval.ModifyingAdmin = userName;
                        userKYCApproval.ModifyingBranch = UserBranch;
                        userKYCApproval.Remarks = info.Remarks;

                        int ret = CustomerUtils.VerifyKYCReset(userKYCApproval);
                        if (ret == 100)
                        {

                            displayMessage = "Customer " + model.Name + " KYC Status Reset has been successfully updated.";
                            messageClass = CssSetting.SuccessMessageClass;
                    ...
                    {
                        this.TempData["Customermodify_messsage"] = "Unauthorized execution";
                        this.TempData["message_class"] = CssSetting.FailedMessageClass;
            ...
            catch (Exception ex)
            {
                ...
                messageClass = CssSetting.FailedMessageClass;
                return RedirectToAction("CustomerList", new { id = Id });
        ...
        public ActionResult VerifiedKYCList(string Key, string Value)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
            ...
            if (TempData["userType"] != null && checkRole)
            {
                ...
                UserInfo userInfo = new UserInfo();
                DataTable dtblUnapproveCus = new DataTable();
                DataTable dtblCus = CustomerUtils.GetVerifiedCustKYC("user");
                string COCs = (string)Session["COC"].ToString();
                bool COC;
                if (Boolean.TryParse(COCs, out COC))
                    COC = true;
                ...
                if (dtblUnapproveCus.Rows.Count > 0)
                {
                    ...
                    userInfo.ContactNumber1 = dtblUnapproveCus.Rows[0]["MobileNumber"].ToString();
                    userInfo.Name = dtblUnapproveCus.Rows[0]["Name"].ToString();
                    userInfo.RejectedBy = dtblUnapproveCus.Rows[0]["ResetBy"].ToString();
                    ViewBag.CustClientCode = userInfo.ClientCode;
                    ViewBag.ContactNumber1 = userInfo.ContactNumber1;
                    ViewBag.CName = userInfo.Name;
                    ViewBag.Address = userInfo.Address;
        ...
        public ActionResult ViewVerifiedCustDetail(string id)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
        ...
        public ActionResult CustKYCApprove(UserInfo model, string btnApprove, FormCollection collection, string Remarks)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (btnApprove.ToUpper() == "REJECT KYC RESET")
                {

                    int ret = CustomerUtils.RejectKYCReset(userInfoApproval);
                    if (ret == 100)
                    {

                        displayMessage = "Customer " + model.Name + " KYC Reset Status has been Rejected.";
                        messageClass = CssSetting.SuccessMessageClass;
                    ...
                    return RedirectToAction("VerifiedKYCList");

                }
                else
                if (btnApprove.ToUpper() == "APPROVE KYC STATUS RESET")
                {
                    userInfoApproval.ApprovedBy = userName;
                    int ret = CustomerUtils.KYCResetApprove(userInfoApproval);
                    if (ret == 100)
                    {


                        displayMessage = "KYC Reset Status for user " + userInfoApproval.FName + " has successfully been approved.";


                        //SMS
                        SMSUtils SMS = new SMSUtils();

                        string Message = string.Format("Dear {0},\n Your KYC has been Reset. Please re-fill your Thaili KYC and submit again. \n Thank You -NIBL Thaili", userInfoApproval.FName);
        ...
        #region Get province and district name
