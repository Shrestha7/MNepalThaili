...
namespace MNSuperadmin.Controllers
{
    public class FeeCalCulationController : Controller
    {
        ...
        public ContentResult FeeModifyTable/*AgentCommisionTable*/(DataTableAjaxPostModel model, string Id, string change, string ToExcel)
        {
            ...
            foreach (var item in res)
            {
                ...
                resultset.Details = item.Details;
        ...
        [HttpGet]
        public ActionResult ViewFeeDetail /*ViewServiceCommissionDetail*/(string id)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
        ...
        [HttpPost]
        public ActionResult ViewFeeDetail(string btnApprove, FormCollection collection)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                try
                {
                    if (btnApprove == "Modify")
                    {
                        ...
                        string feeDetail = collection["txtFeeDetail"].ToString();
                        string branchFeeAcc = collection["txtBranchFeeAcc"].ToString();
                        string feeAcc = collection["txtFeeAcc"].ToString();
                        ...
                        userInfoModify.Details = feeDetail;
                        userInfoModify.BranchFeeAc = branchFeeAcc;
                        userInfoModify.FeeAc = feeAcc;
                        ...
                        mnclientnew.Details = userInfoModify.Details;
                        mnclientnew.BranchFeeAc = userInfoModify.BranchFeeAc;
                        mnclientnew.FeeAc = userInfoModify.FeeAc;
                        ...
                        if (dtableSrInfo.Rows.Count == 1)
                        {
                            ...
                            userInfo.Details = dtableSrInfo.Rows[0]["Details"].ToString();
                            userInfo.BranchFeeAc = dtableSrInfo.Rows[0]["BranchFeeAc"].ToString();
                            userInfo.FeeAc = dtableSrInfo.Rows[0]["FeeAc"].ToString();
                            ...
                            mnclientold.Details = userInfo.Details;
                            mnclientold.BranchFeeAc = userInfo.BranchFeeAc;
                            mnclientold.FeeAc = userInfo.FeeAc;
                ...
                //if (isUpdated)
                return RedirectToAction("Modify", "FeeCalculation");
            }
            else
            {
                return RedirectToAction("Index", "Login");
        ...
        [HttpGet]
        public ActionResult ApproveFeeCalList /*ApproveAgentCommissionList*/(string Key, string Value)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
        ...
        public ActionResult EditModified(string CommissionId)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                {
                    this.TempData["custapprove_messsage"] = "Fee Info not found";
                    this.TempData["message_class"] = CssSetting.FailedMessageClass;
                    return RedirectToAction("ApproveFeeCalList");
                }
        ...
        [HttpPost]
        public ActionResult ApproveFeeCal(UserInfo model, string btnApprove)
        {
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (btnApprove.ToUpper() == "REJECT")
                {
                    ...
                    if (userInfoApproval.Remarks != null)
                    {
                        ...
                        if (ret == 100)
                        {
                            displayMessage = "Fee Modification has been Rejected. Please Check Modification Rejected and perform accordingly";
                            messageClass = CssSetting.SuccessMessageClass;
                        }
                        else if (ret == -1 || ret != 100)
                        {
                            displayMessage = "Error while rejecting Fee " + model.Name;
                            messageClass = CssSetting.FailedMessageClass;
                        }
                    }
                    else
                ...
                else if (btnApprove.ToUpper() == "APPROVE")
                {
                    ...
                    if (ret == 100)
                    {
                        displayMessage = "Information modification for Fee " + model.Name + " has successfully been approved.";
                        messageClass = CssSetting.SuccessMessageClass;
                    }
                    else if (ret == -1 || ret != 100)
                    {
        ...
        public ActionResult ViewApproveFee(string FeeID)
        {
            ...
            {
                return RedirectToAction("CreateFee", "FeeCalculation");
            }
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (dtblFee.Rows.Count == 1)
                {
                    ...
                    userInfo.CreatedBy = dtblFee.Rows[0]["CreatedBy"].ToString();
                    userInfo.CreatedDate = dtblFee.Rows[0]["CreatedDate"].ToString();
                }
        ...
        public ActionResult ViewRejectedFee(string FeeID)
        {
            if (string.IsNullOrEmpty(FeeID))
            {
                return RedirectToAction("FeeRejctedList", "FeeCalculation");
            }
            string userName = (string)Session["LOGGED_USERNAME"];
            string clientCode = (string)Session["LOGGEDUSER_ID"];
            string name = (string)Session["LOGGEDUSER_NAME"];
