            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        string Command = @"SELECT FD.*,FT.* FROM dbo.MNFeeDetails FD INNER JOIN MNFeeTable FT ON FD.FeeId = FT.FeeId where FD.IsApproved = 'UnApprove' AND FD.IsModified = 'T' AND (FD.IsRejected='F' OR FD.IsRejected IS Null)";
        ...
        public int AgentRegisterReject(UserInfo objUserInfo, string Rejected)
        {
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = @"UPDATE MNClient SET Status = 'InActive', IsApproved = 'UnApprove', IsModified = 'F', IsRejected='T', RejectedBy= @RejectedBy, Remarks= @Remarks  where ClientCode = @ClientCode";
