...
namespace MNSuperadmin.UserModels
{
    public class PinUserModel
    {
           ...
           List<UserInfo> UserInfos = new List<UserInfo>();
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        Command = @"Select * from v_MNClientDetail where Status in (@Status1,@Status2,@Status3) AND IsApproved=@IsApproved  AND UserType='agent' ";
                        if (!COC)
                        {
        ...
        #region Pin Reset For Customer
        public List<UserInfo> GetUserPinApproveList(string BranchCode, bool COC, string MobileNo)
        {
            List<UserInfo> UserInfos = new List<UserInfo>();
            SqlConnection conn = null;
            string Command = string.Empty;
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        Command = @"Select * from v_MNClientDetail where Status in (@Status1,@Status2,@Status3) AND IsApproved=@IsApproved  AND UserType='user' ";
                        if (!COC)
                        {
                            Command = Command + " AND ModifyingBranch='" + BranchCode + "'";
                        }
        ...
        public int ApproveUserPin(UserInfo model,string Mode)
        {
            ...
            try
            {
                ...
                using (var command = database.GetStoredProcCommand("[s_MNPinResetApp]"))
                {
                    ...
                    database.AddInParameter(command, "@PIN", DbType.String, HashAlgo.Hash(model.PIN));
                    database.AddInParameter(command, "@Password", DbType.String, HashAlgo.Hash(model.Password));
        ...
        public int UpdatePasswordReset(UserInfo model, string status)
        {
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = @"UPDATE MNClient SET Status='SAPR',IsApproved=@IsApproved,ModifiedBy=@ModifiedBy,ModifyingBranch=@ModifyingBranch
                                               Where ClientCode=@ClientCode";
                        cmd.Parameters.AddWithValue("@ClientCode", model.ClientCode);
                        cmd.Parameters.AddWithValue("@Status", status); //PIN Reset
                        cmd.Parameters.AddWithValue("@IsApproved", "UnApprove");
        ...
        public List<UserInfo> GetPasswordAAL(string UserName)
        {
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        Command = @"SELECT * FROM v_MNClientDetail WHERE Status = 'SAPR' AND IsApproved=@IsApproved AND UserType = 'superadmin' ";
                        ...
                        cmd.CommandText = Command;
                        cmd.Parameters.AddWithValue("@Status", "SAPR");
                        cmd.Parameters.AddWithValue("@IsApproved", "UnApprove");
                        cmd.Connection = conn;
                        if (conn.State != ConnectionState.Open)
                        {
        ...
        //Change pin
        public int ResetPin(UserInfo objPasswordInfo)
        {
            Database database;
            int ret;
            ...
            try
            {
                ...
                using (var command = database.GetStoredProcCommand("[s_MNPinResetApp]"))
                {
                    database.AddInParameter(command, "@PIN", DbType.String, HashAlgo.Hash(objPasswordInfo.PIN));
                    database.AddInParameter(command, "@ClientCode", DbType.String, objPasswordInfo.ClientCode);
                    database.AddInParameter(command, "@Mode", DbType.String, objPasswordInfo.Mode);
                    database.AddOutParameter(command, "@RegIDOut", DbType.Int32, objPasswordInfo.RegIDOut);
                    ret = database.ExecuteNonQuery(command);
                    if (objPasswordInfo.Mode.Equals("NPIN", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ret = (int)database.GetParameterValue(command, "RegIDOut");
                    }
                }
