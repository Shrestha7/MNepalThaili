...
namespace MNSuperadmin.Utilities
{
    public static class PinUtils
    {
        ...
        public static List<UserInfo> GetUserPinResetList(string BranchCode, bool COC, string MobileNo)
        {
            return new PinUserModel().GetUserPinApproveList(BranchCode, COC, MobileNo);
        }
        #endregion
        ...
        #region "Member Change Pin"

        public static bool ResetPin(this UserInfo member)
        {
            var objMemberInfo = new UserInfo()
            {
                PIN = member.PIN,
                ClientCode = member.ClientCode,
                Mode = "NPIN" // Change Pin
            };
            return new PinUserModel().ResetPin(objMemberInfo) > 0;
        }
