...
namespace WCF.MNepal
{
    ...
    public class topup
    {
            ...
            else
            {
                ...
                if (message == "Invalid PIN ")
                {
                    ...
                    if (LoginUtils.GetPINBlockTime(mobile)) //check if blocktime is greater than current time 
                    {
                        ...
                        statusCode = "417";
                        mnlg.ResponseStatus(HttpStatusCode.ExpectationFailed, "Invalid Pin");
                        failedmessage = message;
                ...
                if ((statusCode == "200") && (statusMsg == "Success") && (statusCode != "400"))
                {
                    ...
                    if (result == "Success")
                    {
                        ...
                        if ((getMerchantMobile != "") || (getMerchantMobile != null))
                        {
                            ...
                            getMerchantNameForSMS = getMerchantDetails.PassVIdToGetMerchantName(vid);
                        ...
                        if (mnft.valid() && mnftWithMerchant.valid())
                        {
                            ...
                            string retref = validTransactionData.TraceId;

                            if (sc == "31")
                            {
                                transDailyAmt = CustActivityUtils.GetTransDailyAmt(retref);

                                if (transDailyAmt == "")
                                {
                                    transDailyAmt = transaction.Amount.ToString();
                                }
                            }
                            if (sc == "34")
                            {
                                FeeAmountCheck feeAmtCheck = new FeeAmountCheck();
                                transDailyAmt = feeAmtCheck.GetFeeAmountCheck(retref);
                                if (transDailyAmt == "")
                                {
                                    transDailyAmt = transaction.Amount.ToString();
                                }
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        try
                        {
                            ...
                            //old
                            //messagereply += getMerchantNameForSMS + " TOPUP was successful with amount NPR "
                            //                + validTransactionData.Amount + " to " + destmobile + "." + "\n"; //destmobile

                            messagereply += getMerchantNameForSMS + " TOPUP was successful with amount NPR "
                                           + transDailyAmt + " to " + destmobile + "." + "\n"; //destmobile
                            ...
                            //old
                            //messagereplyReceiver += getMerchantNameForSMS + " TOPUP was successful with amount NPR "
                            //                + validTransactionData.Amount + " to " + destmobile
                            //                + "." + "\n";
                            ...
                            messagereplyReceiver += "Thank you. NIBL Thaili";

                            var client = new WebClient();

                            //var content = client.DownloadString("http://smsvas.mos.com.np/PostSMS.ashx?QueueId=&TelecomId=2&KeywordId=3&Password=mnepal120&From=37878&To=" + "977" + mobile + "&message=" + messagereply + "");
                            ...
                            //    var content = client.DownloadString(
                            //        "http://smsvas.mos.com.np/PostSMS.ashx?QueueId=&TelecomId=2&KeywordId=3&Password=mnepal120&From=37878&To="
                            //        + "977" + mobileReceiver + "&message=" + messagereplyReceiver + "");
                            //}
                    ...
                    else if ((response.StatusCode == HttpStatusCode.BadRequest) || (response.StatusCode == HttpStatusCode.InternalServerError) || (statusCode != "200"))
                    {
                        custsmsInfo = new CustActivityModel()
                        {
                            ...
                            RequestMerchant = getMerchantName,
                            DestinationNo = destmobile,
                            Amount = amount, //validTransactionData.Amount.ToString(),
                            SMSStatus = "Failed",
                            SMSSenderReply = "",
        ...
        protected async Task<string> VerifyMerchant(MerchantTransaction merchantTransaction)
        {
            ...
            input.Field1 = "9803200158";    //PAN
            input.Field2 = "218097292128"; //STAN
            input.Field3 = "20201004152330";
            input.Field4 = "20";
        ...
        public string TopupPaymentReverse(string sc, string vid, string mobile, string destmobile, string merchantmobile, string amount, string pin, string retRef)
        {
            ...
            {
                FeeAmountCheck feeAmtCheck = new FeeAmountCheck();
                amount = feeAmtCheck.GetFeeAmountCheck(retRef);
                if (amount == "")
                {
                ...
                //FeeAmountCheck feeAmtCheck = new FeeAmountCheck();
                //string resultfeeAmt = feeAmtCheck.GetFeeAmountCheck(retRef);
                //fundtransferRev.amount = (Convert.ToDecimal(fundtransferRev.amount) - Convert.ToDecimal(resultfeeAmt)).ToString();
            }
                ...
                if (TopUpRev == "T")
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        try
                        {
                            ...
                            string messagereply = "Dear " + CustCheckUtils.GetName(mobile) + "," + "\n";
                            //old
                            //messagereply += getMerchantName + " TOPUP was successfully reversed with amount NPR "
                            ...
                            string messagereplyReceiver = "Dear " + CustCheckUtils.GetName(destmobile) + "," + "\n";
                            //old
                            //messagereplyReceiver += getMerchantName + " TOPUP was successfully reversed with amount NPR "
                            //                        + validTransactionData.Amount
                            //                        + "." + "\n";
        ...
        public string SMSSend(string mobile, string destmobile, int amount, string getMerchantName)
        {
            ...
            try
            {
                //Sender
                string messagereply = "Dear " + CustCheckUtils.GetName(mobile) + "," + "\n";
                messagereply += getMerchantName + " was successfully transferred with amount NPR "
                                + amount + " to " + destmobile
                                + "." + "\n";
                messagereply += "Thank you. NIBL Thaili";
