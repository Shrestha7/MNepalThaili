...
namespace WCF.MNepal.UserModels
{
    public class CustActivityUserModel
    {
        ...
        #region Get Amount from TransDaily After discount
        
        public string GetTransDailyAmt(string retrievalRef)
        {
            string retrievalReference = retrievalRef;
            string Command = "";

            Command = "Select Top 1 Amount,LCYAmount,* from TransDaily (NOLOCK) where TranId = (select TranId from MNResponse (NOLOCK) where RetrievalRef ='" + retrievalRef + "')";
                ...
                using (System.Data.SqlClient.SqlConnection con = new System.Data.SqlClient.SqlConnection(conStr))
                {
                    ...
                    using (System.Data.SqlClient.SqlCommand command = new System.Data.SqlClient.SqlCommand(sql, con))
                    {
                        ...
                        using (System.Data.SqlClient.SqlDataReader rdr = command.ExecuteReader())
                        {
                            ...
                            {
                                int a = -1;
                                decimal b = Math.Round(Convert.ToDecimal(rdr["Amount"]), 2); // Convert.ToDecimal(rdr["Amount"]);
                                b = b * a;
                                result = b.ToString();
                                break;
                            }
