    ...
    public class nea
    {
        ...
        public string loadwalletrequest(Stream input)
        {
            ...
            else
            {
                ...
                WebOperationContext.Current.OutgoingResponse.Headers.Add("Access-Control-Request-Method", "POST,GET");
        ...
        [WebInvoke(Method = "POST",
                   ResponseFormat = WebMessageFormat.Json)]
        public string manualTranEBanking(Stream input)
        {
            StreamReader sr = new StreamReader(input);
            string s = sr.ReadToEnd();
            sr.Dispose();
            ...
            ThreadPool.QueueUserWorkItem(BackgroundTaskWithObject, fundtransfer);
            MNTransactionMaster validTransactionData = new MNTransactionMaster();
            if (da != null)  /*(checkEbankingRequest(soapTransaction))*/
            {
                ...
                else
                {
                    ...
                    if (tid == "")
                    {
                        ...
                        CustCheckUtils.InsertRetrievalReference(soapTransaction, tid);

                    }

                    bool verifyTransaction = InquireTransaction(soapTransaction); //check Transaction from Bank
                    if (verifyTransaction == true)
                    {
                    ...
                    else
                    {
                        ...
                        CustCheckUtils.InsertEBankingResponse(soapTransaction, statusCode);
                    }
                }
            } //Check Transaction from db
            else
        ...
        [WebInvoke(Method = "POST",
                   ResponseFormat = WebMessageFormat.Json)]
        public string GetTransactionDetails(Stream input)
        {
            StreamReader sr = new StreamReader(input);
            string s = sr.ReadToEnd();
            sr.Dispose();
            ...
            if (verifyTransaction == true)
            {
                result = "Success for BID: " + soapTransaction.BID;
            }
            else
            {
        ...
        protected bool InquireTransaction(SoapTransaction soapTransaction)
        {
            bool returnResult = false;
            WebReference.VerifyHeader objVerifyHeader = new WebReference.VerifyHeader();
            WebReference.VerifyThisSoapClient objVerifyThis = new WebReference.VerifyThisSoapClient();
            ...
            bool responseTableCheck = checkEbankingResponse(soapTransaction);
            if (responseTableCheck == false)
            {
                string result = objVerifyThis.InquireTxn(objVerifyHeader, ITC, PRN, Amount);
                ...
                {
                    string BID = result.Substring(17, 5);
                    soapTransaction.BID = BID;
                    returnResult = true;
                }
                else
                {
        ...
        public bool checkEbankingResponse(SoapTransaction transaction)
        {
            bool result = false;
            string CITC = "";
            string CPRN = "";
            string CAmount = "";
            DataTable dtcheckRequest = CustCheckUtils.GetEBankingResponse(transaction.PRN);
            if (dtcheckRequest.Rows.Count == 1)
            {
                foreach (DataRow row in dtcheckRequest.Rows)
                {
