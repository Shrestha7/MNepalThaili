    ...
    internal class dalTransactionManagement
    {
        ...
        internal string GetClientCode(string username)
        {
            try
            {
                ...
                {
                    var _result = db.Query<MNClientExt>("SELECT * FROM MNClientExt (NOLOCK) WHERE UserName =@0", username).SingleOrDefault();
                    if (_result!=null)
                    {
        ...
        internal string GetPIN(string clientCode)
        {
            try
            {
                using(var db=gc.getConnection())
                {
                    var _result = db.Query<MNClient>("SELECT * FROM MNClient (NOLOCK) WHERE ClientCode=@0", clientCode).SingleOrDefault();
                    return _result.PIN;
                }
            }
            catch(Exception ex)
        ...
        internal  List<MNTransactionMaster> GetMyTransaction(string sourcemobile)//source mobile=username=user mobile no
        {
            try
            {
                ...
                using(var db=gc.getConnection())
                {
                    ...
                    tm = db.Query<MNTransactionMaster>(@"SELECT a.*,b.FeatureName as FeatureName,c.StatusName as StatusName FROM MNTransactionMaster a (NOLOCK)
                                                         INNER  JOIN MNFeatureMaster b (NOLOCK) ON a.FeatureCode=b.FeatureCode 
                                                         INNER JOIN  MNStatus c (NOLOCK) ON a.StatusId=c.ID WHERE a.SourceMobile=@0 OR a.DestinationMobile=@1 ORDER BY CreatedDate desc", sourcemobile,sourcemobile).ToList();
                    return tm;
                }
            }
            catch(Exception ex)
        ...
        internal string GetFeatureName(string _featureCode)
        {
            try
            {
                using (var db = gc.getConnection())
                {
                    var result= db.Query<MNFeatureMaster>("SELECT * FROM MNFeatureMaster (NOLOCK) WHERE FeatureCode=@0", _featureCode).SingleOrDefault();
                    if (result != null)
                    {
        ...
        internal string GetStatusName(int _statusCode)
        {
            try
            {
                using(var db=gc.getConnection())
                {
                    var result = db.Query<MNStatus>("SELECT * FROM MNStatus (NOLOCK) WHERE ID=@0", _statusCode).SingleOrDefault();
                    if(result!=null)
                    {
        ...
        internal string GetBankName(string BankCode)
        {
            try
            {
                using (var db = gc.getConnection())
                {
                   var result= db.Query<MNBankTable>("SELECT * FROM MNBankTable (NOLOCK) WHERE BankCode=@0", BankCode).SingleOrDefault();
                    if(result!=null)
                    {
