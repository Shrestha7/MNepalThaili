    ...
    public class dalUserManagement
    {
        ...
        private string GetClientCode(string username)
        {
            try
            {
                ...
                {
                    var clientCode = db.Query<MNClientExt>("SELECT * FROM MNClientExt (NOLOCK) Where UserName=@0", username).SingleOrDefault();
                    if(clientCode!=null)
                    {
        ...
        public bool isAgent(string username)
        {
            ...
            try
            {
                ...
                if (clientCode != null)
                {
                    using (var db = gc.getConnection())
                    {
                        var agent = db.Query<MNAgent>("SELECT * FROM MNAgent (NOLOCK) WHERE ClientCode=@0", clientCode).SingleOrDefault();
                        if (agent != null)
                        {
                            return true;
                        }
