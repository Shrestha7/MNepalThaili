    ...
    public class AgentDetails
    {
        ...
        public MNAgent AgentDetailsByAgentDetailsFromMNClient(MNAgent mnagent)
        {
            ...
            try
            {
                ...
                {
                    getAgentDetails = dataContext.Single<MNAgent>("SELECT * FROM MNAgent (NOLOCK) where ClientCode=@0", mnagent.ClientCode);
        ...
        public MNClient AgentDetailsByAgentDetailsFromAgentId(MNAgent mnagent)
        {
            ...
            try
            {
                if (mnagent.ID != 0)
                {
                    getDetails = dataContext.Single<MNClient>("SELECT * FROM MNClient (NOLOCK) where ClientCode in (select ClientCode from MNAgent (NOLOCK) where ID=@0)", mnagent.ID);
                    if (getDetails != null)
                    {
                        return getDetails;
                    }
