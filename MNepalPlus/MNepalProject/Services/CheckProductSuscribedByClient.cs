...
namespace MNepalProject.Services
{
    public class CheckProductSuscribedByClient:ReplyMessage
    {
        ...
        public CheckProductSuscribedByClient(string clientCode, string featureCode) {
            ...
            try
            {
                var SubscribedProduct = dataContext.Query<MNSubscribedProduct>("SELECT * FROM MNSubscribedProduct (NOLOCK) where ClientCode=@0 and IsDefault=1 and ProductId in(select ProductId from MNFeature (NOLOCK) where FeatureCode=@1)", clientCode, featureCode);
